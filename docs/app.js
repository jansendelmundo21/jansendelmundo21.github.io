/* app.js — UPDATED
   ✅ Watch Demo now DIRECTLY OPENS your Google Drive link in a new tab
   ✅ No more <video> modal for Drive links (Drive videos can’t reliably play in <video>)
   ✅ Still keeps your lightbox + slider for images
*/
...
function openVideo(src) {
  ...
  // ✅ For Drive/online links: redirect to link (what you want)
...
function createProjectCard(project) {
  ...
  // ✅ Demo UI: use <a> tag so it always redirects for Drive links
  const demoLink =
...
async function loadProjects() {
  try {
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="card" style="text-align: center; padding: 40px 20px;">
          <h3 style="margin-bottom: 12px; color: #dc2626;">⚠️ Projects failed to load</h3>
